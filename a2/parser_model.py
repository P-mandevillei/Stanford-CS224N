...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden_weight = nn.Parameter(torch.zeros(size=(n_features*self.embed_size, hidden_size)))
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)
        self.embed_to_hidden_bias = nn.Parameter(torch.zeros(size=(hidden_size,)))
        nn.init.uniform_(self.embed_to_hidden_bias)
        self.dropout = nn.Dropout(dropout_prob)
        self.hidden_to_logits_weight = nn.Parameter(torch.zeros(size=(hidden_size, n_classes)))
        nn.init.xavier_uniform_(self.hidden_to_logits_weight)
        self.hidden_to_logits_bias = nn.Parameter(torch.zeros(size=(n_classes,)))
    ...
    def forward(self, w):
        ...
        linear = x@self.embed_to_hidden_weight + self.embed_to_hidden_bias # batch_size * hidden_size
